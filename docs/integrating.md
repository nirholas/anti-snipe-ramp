# Integrating AntiSnipeRamp

## From Solidity

```solidity
import {AntiSnipeRampHook} from "anti-snipe-ramp/src/hooks/AntiSnipeRampHook.sol";
...
hook.configure(key, AntiSnipeRampHook.Config({ /* see the README */ }));
...
const address = hookAddress("anti-snipe-ramp", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "AntiSnipeRamp"
...
More at https://anti-snipe-ramp.pages.dev.
