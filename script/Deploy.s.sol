...
/**
 * @title DeployAntiSnipeRamp
 * @notice Deploys AntiSnipeRamp deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployAntiSnipeRamp is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(AntiSnipeRampHook).creationCode;
        ...
        console2.log("AntiSnipeRamp ", predicted);
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("AntiSnipeRamp")),
